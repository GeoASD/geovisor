...
//Enlazar mapas base
var osm_estandar = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
	maxZoom: 19,				 
}).addTo(map);
...
async function fileToGeoJSON(file) {
  ...
  // Manejo de archivos GeoJSON
  if (ext === 'geojson' || ext === 'json') {
    const text = await file.text();
    return JSON.parse(text);

  // Manejo de archivos Shapefile comprimidos (.zip)
  } else if (ext === 'zip') {
    const zip = await JSZip.loadAsync(await file.arrayBuffer());
    const shpFile = zip.file(/\.shp$/i)[0];  // Buscamos el archivo .shp
    if (!shpFile) throw new Error("No se encontró archivo .shp dentro del ZIP");

    // Cargamos el shapefile y lo convertimos a GeoJSON
    const shpData = await shpFile.async("arraybuffer");
    return shp(shpData);

  // Manejo de archivos KML
  ...
  // Manejo de archivos Excel (XLSX)
    ...
    const features = jsonData.map(row => ({
      ...
      geometry: {
        type: 'Point',
        coordinates: [
          parseFloat(row.lon || row.Lon || row.x || row.X),
          parseFloat(row.lat || row.Lat || row.y || row.Y)
        ]
      },
      properties: row
    }));
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Al hacer clic en ejecutar análisis
  runBtn.addEventListener('click', async () => {
    ...
    results.filter(r => r !== null).forEach(({ layerName, result }) => {
	  console.log('Resultado del análisis:', result); // Verifica los resultados de la intersección
	  
	  // Verifica si el tipo de intersección es válido
	  if (result && result.type === 'intersect') {
		console.log('Superposición encontrada');

		// Verifica que los atributos y el área sean accesibles
		const area = result.area ? result.area.toFixed(2) : 'N/A';
		resultsDiv.innerHTML += `
		  ...
		  <div style="border:1px solid green; padding:8px; margin-bottom:8px;">
			...
			Área: ${area} m²<br>
			Atributos:<br>
			<ul>${Object.entries(result.attributes)
			  .slice(0, 5)
			  .map(([k, v]) => `<li>${k}: ${v}</li>`)
			  .join('')}
			</ul>
		...
		console.log('No hay superposición, calculando distancia mínima');
		const distance = result && result.distance_m ? result.distance_m.toFixed(2) : 'N/A';
		resultsDiv.innerHTML += `
		  <strong>${layerName}</strong>
		  <div style="border:1px solid red; padding:8px; margin-bottom:8px;">
			No hay superposición.<br>
			Distancia mínima: ${distance} m<br>
			Atributos del más cercano:<br>
			<ul>${result && result.attributes ? Object.entries(result.attributes)
			  .slice(0, 5)
			  .map(([k, v]) => `<li>${k}: ${v}</li>`)
			  .join('') 
